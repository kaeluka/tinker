...
**Before sending `@goal-id`, ensure you understand the recipient's role.** The compact index and edge reasons are your primary signal. If they're not sufficient, ask `@tend` — tend holds the full goal tree and can describe what any agent does and needs.
...
The compact index and edge reasons tell you what an agent is *responsible for* — enough to write a useful message. They do not answer questions the agent is better positioned to answer.
