...
###### Great-grandchild: `batch-review`
An optional post-batch compliance review step owned by tend. The review covers spec satisfaction, security coverage, and a general scan; rummage reports findings back via `@tend`. Being retired as part of the `goal-agents` session-registry transition — batch-summary machinery (`build_batch_summary_request`, `SummarizingBatch`) has been removed from `main.rs`; `goal-agents` replaces the post-batch flow with goal agents reporting via `@tend` directly. Goal file and Rust tests remain pending cleanup.

📁 `src/tend.rs` — `test_spec_tinker_prompt_describes_batch_review_with_rummage`, `test_spec_tinker_batch_review_selective_not_automatic`, `test_spec_tinker_batch_review_incorporates_findings` (stale; pending removal)  
📁 `src/rummage.rs` — `test_spec_rummage_compliance_review_mode_named`, `test_spec_rummage_compliance_review_covers_three_areas`, `test_spec_rummage_compliance_review_reports_to_tend`, `test_spec_rummage_compliance_review_no_fixes_or_case2` (stale; pending removal)  
📁 `.tinker/goals/batch-review.toml` — goal definition (being retired)
...
The design principle governing how tinker's agent profiles are assigned and kept complementary. Tasks go to the agent whose profile gives the highest likelihood of a high-quality response for that workload. Current profile assignments by cycle position: **tend** (intent↔spec, forward) — intent synthesis and dialectical interviewing; **goal sessions** (spec↔code, forward) — autonomous code generation against a written spec; **rummage** (spec↔code, backward) — grounded code investigation, hypothesis loops, falsification, LSP traversal; **jog** (intent↔spec, backward) — Socratic deepening and no-cue questioning. Intentional overlap exists at the spec-satisfaction boundary: tend reads session reports and synthesizes user-facing output; rummage owns code-reality assessment. An audit mandate fires whenever any agent's profile changes, a new agent is added, or a new recurring consultation pattern emerges. Peer consultation (via `peer-consult`) is only as valuable as the quality gap it crosses; routing to the wrong profile produces worse output than staying put.
...
**`@goal-id message`** is universal dispatch — the mechanism through which any agent activates any other. `/run` is retired; `parse_run_commands` is deleted and all goals have been updated to use `@`-dispatch language. `/jog-edit` is unaffected.
...
### Root: `triggers`
Two dispatch channels plus one advisory. **`@goal-id message`**: the universal dispatch — any agent activates any other by emitting an `@goal-id` block in its reply. After the reply finalizes, the system routes each block to the named recipient's channel via the session registry (`dispatch_peer_consultations`). `/run` is retired; `parse_run_commands` has been deleted. **`/jog-edit <goal-id> <instruction>`**: the jog→tend spec-edit channel. Jog emits this (never `@goal-id`) when an "I know better" finding warrants a goal change; tend applies the edit directly and shows the user what changed.

Context carried in `@goal-id` dispatches from tend is a **declarative pointer to a spec delta** — what changed in the goal or its environment — never an imperative. The `/jog-edit` instruction is prescriptive rather than declarative, which is allowed because it carries user-sourced intent gathered in jog's deepening conversation, not a laundered implementation guess.
...
📁 `src/main.rs` — `parse_at_commands` (updated to accept any registry-known session ID), `dispatch_peer_consultations`, keyboard handler `/run` parsing (still present for user-typed triggers during transition), `parse_jog_edit_commands`; `test_spec_dispatched_run_uses_triggered_system_message_format`, `test_spec_parse_jog_edit_commands_syntax`, `test_spec_jog_edit_unknown_goal_id_silently_skipped`  
