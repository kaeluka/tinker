...
### backends
Pluggable LLM backend abstraction: `OpenCodeRunner` trait wired at the composition root, three implementations (opencode default with system prompt delivered via ephemeral agent file; claude via `--claude` flag with streaming JSON; native via `--native` flag — direct OpenRouter API with an in-process tool loop and per-role `ToolPolicy` enforcement), per-backend model-tier defaults, persona delivery, CLI-error re-injection. `--default-model` bypasses tier resolution, using the opencode backend's built-in default model for every role. `--native` requires `OPENROUTER_API_KEY` and is mutually exclusive with `--claude`; the native backend is the designated successor — the CLI backends are slated for removal once it works well.
...
- `src/native.rs` — `NativeRunner` (native backend); direct OpenRouter chat-completions calls (`reqwest`, key from `OPENROUTER_API_KEY`); in-process tool loop with exactly six tools (bash, read, write, edit, glob, grep); `ToolPolicy` enum (`Unrestricted` for goal sessions including cleanup, `TendScope` for tend — no bash, write/edit only under `.tinker/goals/`, denials returned to the model as tool errors; tend's schema list also omits bash entirely); in-memory session store (`nat_<uuid>` ids; restart = fresh sessions); context overflow drops the session and returns `Err` so `goal_agent_loop` restarts fresh; same struct-level vs per-call system-prompt precedence as `ClaudeRunner`; model constants are OpenRouter ids (no `openrouter/` prefix)

### model-config
User-editable `.tinker/config.toml`: nine optional per-backend tier slots (high/mid/low for opencode, claude, and native), absent slots fall back to built-in defaults, self-documenting starter template written once on first run.
