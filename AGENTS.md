...
- `src/native.rs` — `NativeRunner` (the designated sole backend); direct OpenRouter chat-completions calls (`reqwest`, key from `OPENROUTER_API_KEY`); in-process tool loop with exactly six tools (bash, read, write, edit, glob, grep); `ToolPolicy` enum (`Unrestricted` for goal sessions including cleanup, `TendScope` for tend — restricted tool set with a narrowed write scope on `.tinker/goals/`, denials returned to the model as tool errors; tend's schema list omits bash entirely); `path_within()` performs permission checks with canonical-path resolution when possible (`fs::canonicalize` on existing targets, parent-directory canonicalization for new files) and a pure lexical fallback (`lexical_normalize()`) when the filesystem can't resolve; `extract_assistant_text()` handles both string and array content formats from the OpenAI spec; in-memory session store (`nat_<uuid>` ids; restart = fresh sessions); context overflow drops the session and returns `Err` so `goal_agent_loop` restarts fresh; model constants are OpenRouter ids (no `openrouter/` prefix). **Note:** this file also carries `tend-write-restriction`'s correctness property — equivalent filesystem paths produce the same permission outcome.
