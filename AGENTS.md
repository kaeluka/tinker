...
Runs tend under a custom `opencode` agent profile (`tend`) that strips out the default software-engineer persona via `mode: primary`. Two agent files are written at startup (opencode backend only, always overwritten): `tend.md` carries `TEND_FRONTMATTER` (allowlist: deny all reads/edits/writes except `.tinker/goals/**`) plus the full tend description as its body; `tinker.md` carries `GOAL_AGENT_FRONTMATTER` (denylist: allow all reads/edits/writes except `.tinker/**`) and is used by all non-tend goal-agent sessions.

📁 `src/opencode.rs` — `--agent` flag passed to opencode CLI, `RealOpenCodeRunner::with_agent`  
📁 `src/main.rs` — `GOAL_AGENT_FRONTMATTER`, `TEND_FRONTMATTER`, `tend_agent_content`; `test_spec_tinker_agent_file_frontmatter_is_primary_mode`, `test_spec_tinker_agent_does_not_deny_write_edit_bash`, `test_spec_tinker_md_denies_tinker_dir_access`, `test_spec_tend_md_restricts_to_tinker_goals`, `test_spec_tinker_static_persona_in_agent_dynamic_goals_in_init`, `test_spec_agent_file_always_overwritten_not_guarded_by_exists_check`, `test_spec_tinker_prompt_forbids_vcs_mutation`
...
**Framework preamble** injected before every goal's description: (1) identity — "You are the agent for goal `<id>`"; (2) navigation — compact goal index with `@tend` escalation for goal content not covered by the index; (3) message-passing semantics — how `@goal-id` routing works, actor model (no blocking, no call stack). The agent's role in the cycle is part of its `description`, not runtime-injected.
...
#### Child: `no-peek`
Governs the agents-ask-not-read principle: agents access peer context through `@`-messages only, never by opening another goal's file directly. The edge graph (`[[related]]` and `[[children]]` links with reasons) is the complete coupling map — adding one edge A↔B propagates connectivity to every agent reachable through either endpoint without any node updating its own content. When the compact index isn't sufficient, agents ask; the receiving agent answers or asks follow-ups. Tend is exempt as the goal tree's keeper; all other agents route goal-content questions through `@tend`. Related to `goal-complementarity`: both say coupling lives in graph structure rather than what each node enumerates.

📁 `packaged-goals/no-peek.toml` — goal definition (packaged)
...
### Root: `backends`
Tinker runs against a pluggable backend — the LLM CLI it shells out to — chosen at startup. Two backends exist today: **opencode** (the default) and **claude** (selected with `--claude`). Both are co-equal implementations of the same runner trait, wired to real instances only at the composition root; business logic depends on the trait, not on any vendor's CLI. The opencode backend writes two agent files at startup (always overwriting): `tend.md` uses `TEND_FRONTMATTER` (allowlist model — deny all, allow only `.tinker/goals/**`) plus the full tend description as its body; `tinker.md` uses `GOAL_AGENT_FRONTMATTER` (denylist model — allow all except `.tinker/**`) and is loaded by all other goal-agent sessions. Both constants and `tend_agent_content` are defined in `src/main.rs`. The claude backend maps roles to Opus (tend) and Sonnet (goal sessions), passes the persona via `--system-prompt`, resumes sessions with `--resume <session-id>`, parses streaming JSON (`--output-format stream-json --verbose`), and re-imposes the same `task`/`todowrite` denials mechanically via `--disallowedTools` — keeping the permission boundary identical across both backends. Which model fills each tier is overridable via `model-config`.
...
- **Three chat agents**: tend, rummage, and jog are peer agents sharing the REPL pane. No dedicated source files for rummage or jog — their descriptions come from the packaged goal TOML files via `session_init_message`; they run under the `tinker.md` agent profile (`GOAL_AGENT_FRONTMATTER`). Tend runs under `tend.md` (`TEND_FRONTMATTER` + description body). Both files are written from `src/main.rs` at startup. The active agent is tracked in `App.active_agent`; `/rummage`, `/tend`, and `/jog` slash commands switch between them.
...
- **Security**: documented in `security.md`. Key mitigations include per-file goal loading isolation (T1), per-project session IDs (T2), ancestor-merger transparency (T3), no permission-bypass flags (T4), stderr piped and captured from opencode subprocesses, nulled from claude subprocesses (T5).
