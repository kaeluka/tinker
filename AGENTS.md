...
- `src/main.rs` — `FreshSessionConfig` (carries `session_id`, `dispatcher_id`, `label`, neighbor list); `parse_fresh_dispatches()` (regex-based extraction of fresh-dispatch envelopes; only guard is the ephemeral-depth limit — sessions with `~` in their ID cannot spawn further sub-sessions; interactive agents tend/rummage/jog can spawn freely); ephemeral spawn path in the main loop (unique `~counter` id, inserts into `session_senders` and `app.ephemeral_sessions`, feeds task via `try_send`); retire step that removes completed ephemeral entries from `session_senders` and `app.ephemeral_sessions`
